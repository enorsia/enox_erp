...
@section('title', 'View Return Reason Type — ' . $returnReasonType->name)
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2 mt-0.5">
                        ...
                        @endif
                        <span class="text-[11px] text-slate-400 dark:text-slate-500">Sort: {{ $returnReasonType->sort_order }}</span>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="section-card">
                    ...
                    <div class="divide-y divide-slate-100 dark:divide-slate-700/60">
                        ...
                        <div class="flex items-center justify-between py-3 first:pt-0">
                            ...
                            <span class="text-[13px] text-slate-800 dark:text-slate-100 font-medium text-right">{{ $returnReasonType->name }}</span>
                        </div>
                        ...
                        <div class="flex items-center justify-between py-3">
                            ...
                            <code class="text-[12px] text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded font-mono">{{ $returnReasonType->slug }}</code>
                        </div>
                        ...
                        <div class="flex items-center justify-between py-3">
                            ...
                            <span class="text-[13px] text-slate-700 dark:text-slate-200 font-mono">{{ $returnReasonType->sort_order }}</span>
                        </div>
                        ...
                        @if ($returnReasonType->description)
                            <div class="py-3 last:pb-0">
                                ...
                                <p class="text-[13px] text-slate-700 dark:text-slate-200 leading-relaxed whitespace-pre-wrap">{{ $returnReasonType->description }}</p>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="space-y-5">
                    ...
                    <div class="space-y-3">
                        ...
                        <div>
                            ...
                            <p class="text-[13px] text-slate-700 dark:text-slate-200">
                                {{ $returnReasonType->created_at ? $returnReasonType->created_at->format('M d, Y') : 'N/A' }}
                            </p>
                            <p class="text-[11px] text-slate-400 dark:text-slate-500">
                                {{ $returnReasonType->created_at ? $returnReasonType->created_at->format('H:i') : '' }}
                                @if($returnReasonType->created_at) · {{ $returnReasonType->created_at->diffForHumans() }} @endif
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="text-[13px] text-slate-700 dark:text-slate-200">
                                {{ $returnReasonType->updated_at ? $returnReasonType->updated_at->format('M d, Y') : 'N/A' }}
                            </p>
                            <p class="text-[11px] text-slate-400 dark:text-slate-500">
                                {{ $returnReasonType->updated_at ? $returnReasonType->updated_at->format('H:i') : '' }}
                                @if($returnReasonType->updated_at) · {{ $returnReasonType->updated_at->diffForHumans() }} @endif
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3">
                        ...
                        <p class="text-xl font-bold text-slate-800 dark:text-slate-100">{{ $returnReasonType->sort_order }}</p>
                    </div>
                    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3">
