...
<div x-data="{
    ...
    <div x-show="drawerOpen"
        ...
        <form method="get" action="{{ route('admin.return-reason.index') }}" class="flex-1 flex flex-col overflow-hidden">
            ...
            <div class="flex gap-2.5 px-5 py-4 border-t border-slate-200 dark:border-slate-700 flex-shrink-0">
                <a href="{{ route('admin.return-reason.index') }}" class="flex-1 py-2.5 text-[13px] text-center border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-700 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-600 transition-colors font-medium">Reset</a>
        ...
        <div class="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md border border-slate-200 dark:border-slate-700"
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-[15px] font-semibold text-slate-800 dark:text-slate-100">Export Return Reason</h3>
                </div>
            ...
            <div class="flex gap-2.5 px-6 py-4 border-t border-slate-200 dark:border-slate-700">
                ...
                <a :href="'{{ route('admin.return-reason.export') }}?' + new URLSearchParams(Object.assign({}, Object.fromEntries(new URLSearchParams('{{ http_build_query(request()->except('page')) }}')), {columns: selectedCols.join(',')})).toString()"
    ...
    <div class="p-5 lg:p-6">
        ...
        <div class="flex items-center justify-between mb-5 flex-wrap gap-3">
            <div>
                <h1 class="text-xl font-semibold text-slate-800 dark:text-slate-100">Return Reason</h1>
                <p class="text-sm text-slate-400 dark:text-slate-500 mt-0.5">Manage all return reason and their settings</p>
            </div>
        ...
        @if(request('search') || (request('is_active') !== null && request('is_active') !== ''))
            <div class="flex flex-wrap gap-2 mb-4">
                ...
                @endif
                <a href="{{ route('admin.return-reason.index') }}" class="flex items-center gap-1 text-[11px] text-slate-400 hover:text-red-500 px-2 py-1 rounded-full hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
                    ...
                    <div class="order-card bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4 grid grid-cols-[1fr_auto] gap-3 items-center">
                        ...
                        <div class="flex gap-1 flex-shrink-0">
                            ...
                            @can('general.return_reason_type.delete')
                                ...
                                <form id="delete-form-{{ $reasonType->id }}" method="POST" action="{{ route('admin.return-reason.destroy', $reasonType->id) }}" style="display:none;">
                                    @csrf @method('DELETE')
                                </form>
                            @endcan
                        </div>
