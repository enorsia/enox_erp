...
@section('title', 'Daily Sale Details')
    ...
    <div class="max-w-5xl mx-auto px-5 py-6 pb-28">
        ...
        <div class="flex items-start justify-between mb-6 flex-wrap gap-3">
            <div class="flex items-center gap-3">
                <div class="w-11 h-11 rounded-xl bg-green-50 dark:bg-green-900/20 flex items-center justify-center shrink-0">
                ...
                <div>
                    <h1 class="text-xl font-semibold text-slate-800 dark:text-slate-100">
                        {{ $dailySale->salePlatform->name ?? 'N/A' }}
                    </h1>
                    <div class="flex items-center gap-2 mt-0.5">
                        <span class="badge-custom badge-blue text-[10px]">Daily Sale</span>
                        <span class="text-[11px] text-slate-400 dark:text-slate-500">
                            {{ $dailySale->date ? $dailySale->date->format('d M Y') : 'N/A' }}
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-5">
            ...
            <div class="space-y-5">
                ...
                <div class="section-card">
                    ...
                    <div class="divide-y divide-slate-100 dark:divide-slate-700/60">
                        @php
                            $coreFields = [
                                'Platform'   => $dailySale->salePlatform->name ?? 'N/A',
                                'Date'       => $dailySale->date ? $dailySale->date->format('d M Y') : 'N/A',
                                'Spent'      => number_format($dailySale->spent, 2),
                                'Sales'      => number_format($dailySale->sales, 2),
                                'Orders'     => number_format($dailySale->number_of_orders),
                                'Quantities' => number_format($dailySale->number_of_quantities),
                            ];
                        @endphp
                        @foreach($coreFields as $label => $value)
                            <div class="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                                <span class="text-[12px] font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide w-32 shrink-0">{{ $label }}</span>
                    ...
                    <div class="overflow-x-auto mt-3">
                        <table class="w-full text-sm">
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-700/60">
                                <tr>
                                    <td class="py-2 text-[13px] text-slate-600 dark:text-slate-300">Orders</td>
                                    <td class="py-2 text-right text-[13px] text-slate-700 dark:text-slate-200">{{ number_format($dailySale->number_of_male_orders) }}</td>
                                    <td class="py-2 text-right text-[13px] text-slate-700 dark:text-slate-200">{{ number_format($dailySale->number_of_female_orders) }}</td>
                                    <td class="py-2 text-right text-[13px] text-slate-700 dark:text-slate-200">{{ number_format($dailySale->number_of_kids_orders) }}</td>
                                </tr>
                                <tr>
                                    <td class="py-2 text-[13px] text-slate-600 dark:text-slate-300">Quantities</td>
                                    <td class="py-2 text-right text-[13px] text-slate-700 dark:text-slate-200">{{ number_format($dailySale->number_of_male_quantities) }}</td>
                                    <td class="py-2 text-right text-[13px] text-slate-700 dark:text-slate-200">{{ number_format($dailySale->number_of_female_quantities) }}</td>
                                    <td class="py-2 text-right text-[13px] text-slate-700 dark:text-slate-200">{{ number_format($dailySale->number_of_kids_quantities) }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
            ...
            <div class="space-y-5">
                <div class="section-card">
                    ...
                    <div class="space-y-3">
                        <div>
                            <p class="text-[11px] uppercase tracking-wider text-slate-400 dark:text-slate-500 font-medium mb-1">Created</p>
                            <p class="text-[13px] text-slate-700 dark:text-slate-200">
                                {{ $dailySale->created_at ? $dailySale->created_at->format('d M Y H:i') : 'N/A' }}
                            </p>
                            <p class="text-[11px] text-slate-400">{{ $dailySale->created_at?->diffForHumans() }}</p>
                        </div>
                        <div>
                            <p class="text-[11px] uppercase tracking-wider text-slate-400 dark:text-slate-500 font-medium mb-1">Last Updated</p>
                            <p class="text-[13px] text-slate-700 dark:text-slate-200">
                                {{ $dailySale->updated_at ? $dailySale->updated_at->format('d M Y H:i') : 'N/A' }}
                            </p>
                            <p class="text-[11px] text-slate-400">{{ $dailySale->updated_at?->diffForHumans() }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="sticky-footer mt-5 -mx-5 rounded-none">
            <div class="max-w-5xl mx-auto flex items-center justify-between gap-3 flex-wrap">
                <div class="flex items-center gap-2 text-xs text-slate-400 dark:text-slate-500">
                    ...
                    <span>ID: <code class="font-mono text-slate-500">{{ $dailySale->id }}</code></span>
                </div>
                <div class="flex gap-2.5">
                    ...
                    @can('general.daily_sale.edit')
                        <a href="{{ route('admin.daily-sales.edit', $dailySale->id) }}"
