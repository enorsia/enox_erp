                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700/60">
                        @forelse ($dailySales as $key => $sale)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                                ...
                                <td class="px-4 py-3"><span class="text-[13px] font-medium text-slate-700 dark:text-slate-200">{{ $sale->salePlatform->name ?? 'N/A' }}</span></td>
                                <td class="px-4 py-3 text-[13px] text-slate-600 dark:text-slate-300">{{ $sale->date ? $sale->date->format('d M Y') : 'N/A' }}</td>
                                <td class="px-4 py-3 text-right text-[13px] text-slate-600 dark:text-slate-300">{{ number_format($sale->spent, 2) }}</td>
                                <td class="px-4 py-3 text-right text-[13px] font-semibold text-slate-800 dark:text-slate-100">{{ number_format($sale->sales, 2) }}</td>
                                <td class="px-4 py-3 text-right text-[13px] text-slate-600 dark:text-slate-300">{{ number_format($sale->number_of_orders) }}</td>
                                <td class="px-4 py-3 text-right text-[13px] text-slate-600 dark:text-slate-300">{{ number_format($sale->number_of_quantities) }}</td>
                                <td class="px-4 py-3">
                        ...
                        @empty
                            <tr><td colspan="8" class="px-4 py-10 text-center text-sm text-slate-400 dark:text-slate-500">No daily sales records found.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
