...
@section('title', 'Add Daily Return')
...
@section('content')
    ...
    <div class="max-w-5xl mx-auto px-5 py-6 pb-28">
        ...
        <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
            <div>
                <h1 class="text-xl font-semibold text-slate-800 dark:text-slate-100">Add Daily Return</h1>
                <p class="text-sm text-slate-400 dark:text-slate-500 mt-0.5">Record a new daily returns entry</p>
            </div>
        </div>

        <form method="POST" action="{{ route('admin.daily-returns.store') }}" id="validateForm">
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-5">
                ...
                <div class="space-y-5">
                    ...
                    <div class="section-card">
                        ...
                        <p class="section-desc">Select platform, reason, and date, then enter return totals.</p>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <select name="return_reason_type_id" class="tom-select f-input @error('return_reason_type_id') border-red-400 @enderror" required>
                                    ...
                                    @foreach($reasonTypes as $reason)
                                        <option value="{{ $reason->id }}" {{ old('return_reason_type_id') == $reason->id ? 'selected' : '' }}>
                                            {{ $reason->name }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('return_reason_type_id')
                                    <p class="f-error">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="number_of_returns" min="0"
                                       ...
                                       placeholder="0" value="{{ old('number_of_returns', 0) }}" required />
                                @error('number_of_returns')
                                    <p class="f-error">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="number_of_return_quantities" min="0"
                                       ...
                                       placeholder="0" value="{{ old('number_of_return_quantities', 0) }}" required />
                                @error('number_of_return_quantities')
                                    <p class="f-error">{{ $message }}</p>
                                @enderror
                            </div>
                        </div>
                    ...
                    <div class="section-card">
                        ...
                        <p class="section-desc">Optional breakdown of returns by gender.</p>

                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                <label class="f-label">Male Returns</label>
                                <input type="number" name="number_of_male_returns" min="0"
                                       class="f-input @error('number_of_male_returns') border-red-400 @enderror"
                                       placeholder="0" value="{{ old('number_of_male_returns', 0) }}" />
                                @error('number_of_male_returns') <p class="f-error">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label class="f-label">Female Returns</label>
                                <input type="number" name="number_of_female_returns" min="0"
                                       class="f-input @error('number_of_female_returns') border-red-400 @enderror"
                                       placeholder="0" value="{{ old('number_of_female_returns', 0) }}" />
                                @error('number_of_female_returns') <p class="f-error">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label class="f-label">Kids Returns</label>
                                <input type="number" name="number_of_kids_returns" min="0"
                                       class="f-input @error('number_of_kids_returns') border-red-400 @enderror"
                                       placeholder="0" value="{{ old('number_of_kids_returns', 0) }}" />
                                @error('number_of_kids_returns') <p class="f-error">{{ $message }}</p> @enderror
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="section-card">
                        ...
                        <p class="section-desc">Optional breakdown of return quantities by gender.</p>

                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                <label class="f-label">Male Qty</label>
                                <input type="number" name="number_of_male_return_quantities" min="0"
                                       class="f-input @error('number_of_male_return_quantities') border-red-400 @enderror"
                                       placeholder="0" value="{{ old('number_of_male_return_quantities', 0) }}" />
                                @error('number_of_male_return_quantities') <p class="f-error">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label class="f-label">Female Qty</label>
                                <input type="number" name="number_of_female_return_quantities" min="0"
                                       class="f-input @error('number_of_female_return_quantities') border-red-400 @enderror"
                                       placeholder="0" value="{{ old('number_of_female_return_quantities', 0) }}" />
                                @error('number_of_female_return_quantities') <p class="f-error">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label class="f-label">Kids Qty</label>
                                <input type="number" name="number_of_kids_return_quantities" min="0"
                                       class="f-input @error('number_of_kids_return_quantities') border-red-400 @enderror"
                                       placeholder="0" value="{{ old('number_of_kids_return_quantities', 0) }}" />
                                @error('number_of_kids_return_quantities') <p class="f-error">{{ $message }}</p> @enderror
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-5">
                    <div class="section-card">
                        ...
                        <ul class="text-[12px] text-slate-500 dark:text-slate-400 space-y-2 mt-2">
                            <li>• Each platform can have only one return record per day per reason.</li>
                            <li>• Gender breakdown fields are optional.</li>
                        </ul>
                    </div>
                </div>
            </div>
