...
<div x-data="{
    ...
    <div x-show="drawerOpen"
        ...
        <form method="get" action="{{ route('admin.sale-tracking.index') }}" class="flex-1 flex flex-col overflow-hidden">
            ...
            <div class="flex gap-2.5 px-5 py-4 border-t border-slate-200 dark:border-slate-700 shrink-0">
                <a href="{{ route('admin.sale-tracking.index') }}" class="flex-1 py-2.5 text-[13px] text-center border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-700 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-600 transition-colors font-medium">Reset</a>
    ...
    <div class="p-5 lg:p-6">
        ...
        <div class="flex items-center justify-between mb-5 flex-wrap gap-3">
            ...
            <div class="flex items-center gap-2 flex-wrap">
                ...
                {{-- Export --}}
                <a href="{{ route('admin.sale-tracking.export') }}?{{ http_build_query(request()->except('page')) }}"
                ...
                @can('general.sale_tracking.create')
                    <a href="{{ route('admin.sale-tracking.create') }}"
        ...
        <div class="flex flex-wrap gap-2 mb-4">
            ...
            @endif
            <a href="{{ route('admin.sale-tracking.index') }}" class="flex items-center gap-1 text-[11px] text-slate-400 hover:text-red-500 px-2 py-1 rounded-full hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
        ...
        @if($records->isEmpty())
            <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-12 text-center">
                ...
                @can('general.sale_tracking.create')
                    <a href="{{ route('admin.sale-tracking.create') }}" class="inline-flex items-center gap-2 mt-4 px-4 py-2 text-sm rounded-xl bg-accent-400 hover:bg-accent-600 text-white font-semibold transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" d="M12 5v14M5 12h14"/></svg>
                        Add First Records
                    </a>
                @endcan
                ...
                <div class="flex items-center justify-between flex-wrap gap-3 px-5 py-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-800/80">
                    ...
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        @can('general.sale_tracking.edit')
                            <a href="{{ route('admin.sale-tracking.edit', $mg['entries']->first()->id) }}"
                    ...
                    <div class="group bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700 rounded-xl p-3.5 hover:border-accent-200 dark:hover:border-accent-700 hover:shadow-sm transition-all">
                        ...
                        <div class="flex items-start justify-between gap-2 mb-3">
                            ...
                            <div class="flex gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                                @can('general.sale_tracking.edit')
                                    <a href="{{ route('admin.sale-tracking.edit', $rec->id) }}"
                                ...
                                @can('general.sale_tracking.delete')
                                    ...
                                    <form id="delete-st-{{ $rec->id }}" method="POST"
                                                  action="{{ route('admin.sale-tracking.destroy', $rec->id) }}" style="display:none;">
                                                @csrf @method('DELETE')
                                            </form>
                                @endcan
                            </div>
